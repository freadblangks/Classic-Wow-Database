...
new Listview(
	{ldelim}template:'npc',
		id:'{$id}',
		{if $num_npcs}note:'{$num_npcs} NPC TOTAL',{/if}
		...
		hiddenCols:[{if $cost}'type'{else}'hidden'{/if}],
		sort:[{if $percent}'-percent',{/if} 'name'],
		data:[
			{section name=i loop=$data}
				{ldelim}
					{if $data[i].classification == 3}
					boss:'1',
					{/if}
					...
					{/if}
					minlevel:{$data[i].level_min},
					maxlevel:{$data[i].level_max},
					{if $data[i].location}location:[{$data[i].location}],{else}location:0,{/if}
					type:{$data[i].type},
